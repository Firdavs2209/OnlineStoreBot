...
class Database:
    ...
    def del_cacategory(self,name,id):
        try:
            self.cursor.execute(
                "DELETE FROM cacategory_name=? WHERE id=?",
                (name,id)
            )
            self.conn.commit()
            return True
    ...
    def add_product(self, new_cat):
        product = self.cursor.execute(
            "SELECT id, product_name FROM product WHERE product_name=?;",
            (new_cat,)
        ).fetchone()
    ...
    def edit_product(self, new_name, id):
        try:
            self.cursor.execute(
                "UPDATE procedure SET procedure_name=? WHERE id=?",
                (new_name, id)
            )
            self.conn.commit()
            return True
    ...
    def del_product(self, name, id):
        try:
            self.cursor.execute(
                "DELETE FROM procedure_name=? WHERE id=?",
                (name, id)
            )
            self.conn.commit()
            return True
